...
void AUTAD_UI_FPSCharacter::Tick(float DeltaSeconds)
{
	...
	if (bHit)
	{
		// Perform your conditional check here
		if (Cast<AUTAD_UI_FPS_Enemy>(HitResult.GetActor()))
		{
			//DrawDebugLine(GetWorld(), Start, End, FColor::Red, false, 1, 0, 1);
			Cast<UCrosshair>(GetPlayerHUDInstance()->GetWidgetFromName("CrosshairWidget"))->TurnRed();
		
		}
		else
		{
			Cast<UCrosshair>(GetPlayerHUDInstance()->GetWidgetFromName("CrosshairWidget"))->TurnBlack();
		}
	}
	else
	{
	...
	if(GEngine)
	{
		GEngine->AddOnScreenDebugMessage(100, 15.0f, FColor::Red, FString::Printf(TEXT("SkillPoints: %i"), SkillPoints));
	}
}
...
void AUTAD_UI_FPSCharacter::SetHealth(int NewHealth)
{
	...
	Cast<UPlayerHealthBar>(PlayerHUDInstance->GetWidgetFromName("PlayerHealthBarWidget"))->UpdatePlayerHealthBar(Health, MaxHealth);
}
