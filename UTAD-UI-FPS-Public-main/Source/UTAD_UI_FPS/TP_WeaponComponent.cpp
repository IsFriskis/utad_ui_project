...
void UTP_WeaponComponent::Fire()
{
	...
	if (ProjectileClass != nullptr)
	{
		...
		if (World != nullptr)
		{
			...
			UPlayerHUD* PlayerHUD = Cast<UPlayerHUD>(Character->GetPlayerHUDInstance());
			if(PlayerHUD)
			{
				PlayerHUD->PlayAnimation(PlayerHUD->ShootAnimation, 0.f, 1.0f);
			}
	
			// Spawn the projectile at the muzzle
			AUTAD_UI_FPSProjectile* proj = World->SpawnActor<AUTAD_UI_FPSProjectile>(ProjectileClass, SpawnLocation, SpawnRotation, ActorSpawnParams);
			if(proj)
			{
				proj->SetProjectileOwner(Character);
			}
		}
	}
